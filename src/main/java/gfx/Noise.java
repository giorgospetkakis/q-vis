...
public class Noise {
	...
	private static float[][] simplex(int width, int height) {
		float[][] noise = new float[width][height];
		float frequency = 5.0f / (float) width;

		for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				noise[i][j] = (float) noise(i * frequency, j * frequency);
	...
	private static float[][] gaussian(int width, int height) {
		float[][] noise = new float[width][height];
		...
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				...
				noise[i][j] = (float) (Math.sqrt(-2 * Math.log(u)) * Math.cos(2 * Math.PI * v));
			}
		}
		return noise;
	}

	public static void display(int width, int height, GL2 gl) {
		float[][] noise = gaussian(width, height);

		gl.glBegin(GL.GL_POINTS);
		for (int i = 0; i < width; i++) {
			for (int j = 0; j < height; j++) {
				gl.glColor3f(noise[i][j], noise[i][j], noise[i][j]);
				gl.glVertex2f(i, j);
			}
		}
		gl.glEnd();
